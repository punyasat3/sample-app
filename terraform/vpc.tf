...
resource "aws_subnet" "mysubnet" {
  ...
  tags = {
    Name = "${var.partition_name}-Subnet-Public-${var.env}-${count.index + 1}"
  }
...
resource "aws_subnet" "mysubnet_private" {
  ...
  availability_zone = "${data.aws_availability_zones.azs.names[count.index]}"
  tags = {
    Name = "${var.partition_name}-Subnet-Private-${var.env}-${count.index + 1}"
  }
...
resource "aws_internet_gateway" "my_igw" {
  vpc_id = local.vpc_id
  tags = {
    Name = "${var.partition_name}-IGW-${var.env}"
  }
}
#########################################
...
resource "aws_route_table" "public_rt" {
  ...
  tags = {
    Name = "${var.partition_name}-Public-RT-${var.env}"
  }
}
#########################################
...
resource "aws_route_table" "private_rt_dev" {
  ...
  tags = {
    Name = "${var.partition_name}-Private-RT-${var.env}"
  }
}
#########################################
...
resource "aws_eip" "dev_eip" {
  ...
  count = "${var.env == "dev" ? 1 : 0}"
  tags = {
    Name = "${var.partition_name}-EIP-${var.env}"
  }
}
###################################################
...
resource "aws_nat_gateway" "nat_gateway" {
  ...
  tags = {
    Name = "${var.partition_name}-NAT-GATEWAY-${var.env}"
  }
}
###################################################
